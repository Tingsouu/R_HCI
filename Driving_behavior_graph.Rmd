...
#g3_1 <- ggplot(df, aes(x = factor(condition_road), y = Takeover_time, fill = NDRT)) +
  ...
  #labs(title = "Takeover_time by Condition ",
      #x = "Road",
       #y = "Takeover_time") +
  #theme_minimal() +
  #scale_y_continuous(limits = c(0, max_y + 0.5))  # è®¾ç½® y è½´è?ƒå›´ï¼Œæ?€å¤§å€¼å??0.5
...
ggsave("graph/combined_plot_2 .pdf", plot = combined_plot_time , width = 14, height = 7, dpi = 1000)
...
g_L <- ggplot(df, aes(x = factor(condition_road), y = Large_Steering, fill = NDRT)) +
  ...
  labs(title = "Steering angle by Condition",
       ...
       y = "Steering angle") +
  theme_minimal() +
  theme(plot.title = element_text(size = 16),  # ???¾ã??????¦rÊ^¤j¤p
        axis.title = element_text(size = 14),  # ???¾ã?????????¦rÊ^¤j¤p
        axis.text = element_text(size = 12))   # ???¾ã?????????¦rÊ^¤j¤p

# ¥´¦L???§Î
print(g_L)

# «O¦s???¹³¦}¼W¥[¦rÊ^¤j¤p
ggsave("graph/Large_Steering.pdf", plot = g_L, width = 6, height = 4, dpi = 800)
```
...
# ???¤T??????¦X¦}¦¨¤@??????
combined_plot_s <- grid.arrange(g_Lateral_acceleration, g_Maximum_Deceleration, g_Total_acceleration, ncol = 3)

# «O¦s???¹³
ggsave("graph/combined_plot_s.pdf", plot = combined_plot_s, width = 18, height = 6, dpi = 800)
```
